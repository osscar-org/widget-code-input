...
function checkSelection(selection4, docLength) {
  for (let range2 of selection4.ranges)
...
var Transaction = class _Transaction {
  constructor(startState, changes, selection4, effects, annotations, scrollIntoView3) {
    ...
    this._state = null;
    if (selection4)
      checkSelection(selection4, changes.newLength);
  ...
  @internal
  */
  static create(startState, changes, selection4, effects, annotations, scrollIntoView3) {
    return new _Transaction(startState, changes, selection4, effects, annotations, scrollIntoView3);
  }
  /**
...
var EditorState = class _EditorState {
  constructor(config2, doc2, selection4, values2, computeSlot, tr) {
    this.config = config2;
    this.doc = doc2;
    this.selection = selection4;
    this.values = values2;
  ...
  */
  applyTransaction(tr) {
    ...
    let selection4 = tr.startState.facet(allowMultipleSelections) ? tr.newSelection : tr.newSelection.asSingle();
    new _EditorState(conf, tr.newDoc, selection4, startValues, (state, slot) => slot.update(state, tr), tr);
  }
  /**
  ...
  */
  static create(config2 = {}) {
    ...
    let selection4 = !config2.selection ? EditorSelection.single(0) : config2.selection instanceof EditorSelection ? config2.selection : EditorSelection.single(config2.selection.anchor, config2.selection.head);
    checkSelection(selection4, doc2.length);
    if (!configuration.staticFacet(allowMultipleSelections))
      selection4 = selection4.asSingle();
    return new _EditorState(configuration, doc2, selection4, configuration.dynamicSlots.map(() => null), (state, slot) => slot.create(state), null);
  }
  /**
...
var RangeSet = class _RangeSet {
  ...
  */
  map(changes) {
    ...
    for (let i = 0; i < this.chunk.length; i++) {
      let start = this.chunkPos[i], chunk2 = this.chunk[i];
  ...
  */
  static spans(sets, from, to, iterator, minPointSize = -1) {
    let cursor4 = new SpanCursor(sets, null, minPointSize).goto(from), pos = from;
    let openRanges = cursor4.openStart;
    for (; ; ) {
      let curTo = Math.min(cursor4.to, to);
      if (cursor4.point) {
        let active = cursor4.activeForPoint(cursor4.to);
        let openCount = cursor4.pointFrom < from ? active.length + 1 : cursor4.point.startSide < 0 ? active.length : Math.min(active.length, openRanges);
        iterator.point(pos, curTo, cursor4.point, active, openCount, cursor4.pointRank);
        openRanges = Math.min(cursor4.openEnd(curTo), active.length);
      } else if (curTo > pos) {
        iterator.span(pos, curTo, cursor4.active, openRanges);
        openRanges = cursor4.openEnd(curTo);
      }
      if (cursor4.to > to)
        return openRanges + (cursor4.point && cursor4.to > to ? 1 : 0);
      pos = cursor4.to;
      cursor4.next();
    }
  }
  ...
  static join(sets) {
    ...
    for (let i = sets.length - 2; i >= 0; i--) {
      for (let layer2 = sets[i]; layer2 != _RangeSet.empty; layer2 = layer2.nextLayer)
        result2 = new _RangeSet(layer2.chunkPos, layer2.chunk, result2, Math.max(layer2.maxPoint, result2.maxPoint));
    }
    return result2;
  }
};
...
function hasSelection(dom, selection4) {
  if (!selection4.anchorNode)
    return false;
  try {
    return contains(dom, selection4.anchorNode);
  } catch (_3) {
    return false;
  }
}
...
function atElementStart(doc2, selection4) {
  let node = selection4.focusNode, offset = selection4.focusOffset;
  if (!node || selection4.anchorNode != node || selection4.anchorOffset != offset)
    return false;
...
var MarkView = class _MarkView extends ContentView {
  ...
  split(from) {
    let result2 = [], off = 0, detachFrom = -1, i = 0;
    ...
    for (let i = 0, off = 0; i < view2.children.length && off <= pos2; i++) {
      ...
      if (end >= pos2) {
        if (child.children.length) {
          scan(child, pos2 - off);
        } else if ((!after2 || after2.isHidden && side > 0) && (end > pos2 || off == end && child.getSide() > 0)) {
          after2 = child;
          afterPos = pos2 - off;
  ...
  static extendWithRanges(diff, ranges) {
    ...
    for (let dI = 0, rI = 0, posA = 0, posB = 0; ; dI++) {
      ...
      while (rI < ranges.length && ranges[rI] < end) {
        ...
        if (fromB <= toB)
          new _ChangedRange(fromB + off, toB + off, fromB, toB).addToSet(result2);
        if (to > end)
          break;
        else
          rI += 2;
...
var DocView = class extends ContentView {
  ...
  updateChildren(changes, oldLength, composition) {
    ...
    let cursor4 = this.childCursor(oldLength);
    for (let i = ranges.length - 1; ; i--) {
      ...
      let { i: toI, off: toOff } = cursor4.findPos(toA, 1);
      let { i: fromI, off: fromOff } = cursor4.findPos(fromA, -1);
  ...
  // composition.
  suppressWidgetCursorChange(sel, cursor4) {
    return this.hasComposition && cursor4.empty && isEquivalentPosition(sel.focusNode, sel.focusOffset, sel.anchorNode, sel.anchorOffset) && this.posFromDOM(sel.focusNode, sel.focusOffset) == cursor4.head;
  }
  enforceCursorAssoc() {
    if (this.hasComposition)
      return;
    let { view } = this, cursor4 = view.state.selection.main;
    let sel = getSelection(view.root);
    let { anchorNode, anchorOffset } = view.observer.selectionRange;
    if (!sel || !cursor4.empty || !cursor4.assoc || !sel.modify)
      return;
    let line = LineView.find(this, cursor4.head);
    if (!line)
      return;
    let lineStart = line.posAtStart;
    if (cursor4.head == lineStart || cursor4.head == lineStart + line.length)
      return;
    let before2 = this.coordsAt(cursor4.head, -1), after2 = this.coordsAt(cursor4.head, 1);
    if (!before2 || !after2 || before2.bottom > after2.top)
      return;
    let dom = this.domAtPos(cursor4.head + cursor4.assoc);
    sel.collapse(dom.node, dom.offset);
    sel.modify("move", cursor4.assoc < 0 ? "forward" : "backward", "lineboundary");
    view.observer.readSelectionRange();
    let newRange = view.observer.selectionRange;
    if (view.docView.posFromDOM(newRange.anchorNode, newRange.anchorOffset) != cursor4.from)
...
var MouseSelection = class {
  ...
  select(event) {
    let { view } = this, selection4 = this.skipAtoms(this.style.get(event, this.extend, this.multiple));
    if (this.mustSelect || !selection4.eq(view.state.selection, this.dragging === false))
      this.view.dispatch({
...
var HeightMapGap = class _HeightMapGap extends HeightMap {
  ...
  decomposeLeft(to, result2) {
    result2.push(new _HeightMapGap(to - 1), null);
  }
  ...
  if (change.from >= sel.from && change.to <= sel.to && change.to - change.from >= (sel.to - sel.from) / 3 && (!newSel || newSel.main.empty && newSel.main.from == change.from + change.insert.length) && view.inputState.composing < 0) {
    ...
    if (startState.selection.ranges.length > 1 && view.inputState.composing >= 0 && change.to <= sel.to && change.to >= sel.to - 10) {
      ...
      tr = startState.changeByRange((range2) => {
        ...
        if (range2.to - range2.from != size2 || view.state.sliceDoc(from, to) != replaced || // Unfortunately, there's no way to make multiple
  ...
  markers(view) {
    ...
    for (let r of state.selection.ranges) {
      ...
      if (r.empty ? !prim || CanHidePrimary : conf.drawRangeCursor) {
        ...
        let cursor4 = r.empty ? r : EditorSelection.cursor(r.head, r.head > r.anchor ? -1 : 1);
        for (let piece of RectangleMarker.forRange(view, className, cursor4))
          cursors.push(piece);
      }
    }
    return cursors;
...
function iterMatches(doc2, re, from, to, f) {
  re.lastIndex = 0;
  for (let cursor4 = doc2.iterRange(from, to), pos = from, m; !cursor4.next().done; pos += cursor4.value.length) {
    if (!cursor4.lineBreak)
      while (m = re.exec(cursor4.value))
        f(pos + m.index, m);
  }
}
...
function advanceCursor(cursor4, collect, pos) {
  while (cursor4.value && cursor4.from <= pos) {
    if (cursor4.from == pos)
      collect.push(cursor4.value);
    cursor4.next();
  }
}
...
function buildTree(data) {
  ...
  let cursor4 = Array.isArray(buffer) ? new FlatBufferCursor(buffer, buffer.length) : buffer;
  let types2 = nodeSet.types;
  let contextHash = 0, lookAhead = 0;
  function takeNode(parentStart, minPos, children2, positions2, inRepeat, depth) {
    let { id: id2, start, end, size: size2 } = cursor4;
    ...
    if (end - start <= maxBufferLength && (buffer2 = findBufferSize(cursor4.pos - minPos, inRepeat))) {
      let data2 = new Uint16Array(buffer2.size - buffer2.skip);
      let endPos = cursor4.pos - buffer2.size, index = data2.length;
      while (cursor4.pos > endPos)
        index = copyToBuffer(buffer2.start, data2, index);
      node = new TreeBuffer(data2, end - buffer2.start, nodeSet);
      startPos = buffer2.start - parentStart;
    } else {
      let endPos = cursor4.pos - size2;
      cursor4.next();
      ...
      while (cursor4.pos > endPos) {
        if (localInRepeat >= 0 && cursor4.id == localInRepeat && cursor4.size >= 0) {
          if (cursor4.end <= lastEnd - maxBufferLength) {
            makeRepeatLeaf(localChildren, localPositions, start, lastGroup, cursor4.end, lastEnd, localInRepeat, lookAheadAtStart);
  ...
  function copyToBuffer(bufferStart, buffer2, index) {
    ...
    if (size2 >= 0 && id2 < minRepeatType) {
      ...
      if (size2 > 4) {
        let endPos = cursor4.pos - (size2 - 4);
        while (cursor4.pos > endPos)
...
var NodeWeakMap = class {
  ...
  */
  cursorSet(cursor4, value) {
    if (cursor4.buffer)
      this.setBuffer(cursor4.buffer.buffer, cursor4.index, value);
    else
      this.map.set(cursor4.tree, value);
  }
  /**
...
var HighlightBuilder = class {
  ...
  highlightRange(cursor4, from, to, inheritedClass, highlighters) {
    let { type, from: start, to: end } = cursor4;
    if (start >= to || end <= from)
      return;
    if (type.isTop)
      highlighters = this.highlighters.filter((h) => !h.scope || h.scope(type));
    let cls = inheritedClass;
    let rule = getStyleTags(cursor4) || Rule.empty;
    ...
    if (rule.opaque)
      return;
    let mounted = cursor4.tree && cursor4.tree.prop(NodeProp.mounted);
    if (mounted && mounted.overlay) {
      let inner = cursor4.node.enter(mounted.overlay[0].from + start, 1);
      let innerHighlighters = this.highlighters.filter((h) => !h.scope || h.scope(mounted.tree.type));
      let hasChild2 = cursor4.firstChild();
      for (let i = 0, pos = start; ; i++) {
        let next = i < mounted.overlay.length ? mounted.overlay[i] : null;
        let nextPos = next ? next.from + start : end;
        let rangeFrom = Math.max(from, pos), rangeTo = Math.min(to, nextPos);
        if (rangeFrom < rangeTo && hasChild2) {
          while (cursor4.from < rangeTo) {
            this.highlightRange(cursor4, rangeFrom, rangeTo, inheritedClass, highlighters);
            this.startSpan(Math.min(rangeTo, cursor4.to), cls);
            if (cursor4.to >= nextPos || !cursor4.nextSibling())
              break;
          }
        }
        if (!next || nextPos > to)
      ...
      do {
        ...
        if (cursor4.from >= to)
          break;
        this.highlightRange(cursor4, from, to, inheritedClass, highlighters);
        this.startSpan(Math.min(to, cursor4.to), cls);
      } while (cursor4.nextSibling());
      cursor4.parent();
    }
  }
};
...
function matchMarkedBrackets(_state, _pos, dir, token, handle, matching, brackets) {
  ...
  let depth = 0, cursor4 = parent === null || parent === void 0 ? void 0 : parent.cursor();
  if (cursor4 && (dir < 0 ? cursor4.childBefore(token.from) : cursor4.childAfter(token.to)))
    do {
      if (dir < 0 ? cursor4.to <= token.from : cursor4.from >= token.to) {
        if (depth == 0 && matching.indexOf(cursor4.type.name) > -1 && cursor4.from < cursor4.to) {
          let endHandle = findHandle(cursor4);
          return { start: firstToken, end: endHandle ? { from: endHandle.from, to: endHandle.to } : void 0, matched: true };
        } else if (matchingNodes(cursor4.type, dir, brackets)) {
          depth++;
        } else if (matchingNodes(cursor4.type, -dir, brackets)) {
          if (depth == 0) {
            let endHandle = findHandle(cursor4);
    ...
    } while (dir < 0 ? cursor4.prevSibling() : cursor4.nextSibling());
  ...
  createListBox(options, id2, range2) {
    ...
    for (let i = range2.from; i < range2.to; i++) {
      ...
      if (section) {
        ...
        if (name2 != curSection && (i > range2.from || range2.from == 0)) {
          curSection = name2;
  ...
  let dont = null, changes = state.changeByRange((range2) => {
    if (range2.empty) {
      ...
      for (let token of tokens) {
        if (token == before2 && nextChar(state.doc, range2.head) == closing(codePointAt(token, 0)))
          return {
...
function cmd(side, selection4) {
  return function({ state, dispatch }) {
    if (!selection4 && state.readOnly)
      return false;
    let historyState = state.field(historyField_, false);
    if (!historyState)
      return false;
    let tr = historyState.pop(side, state, selection4);
    if (!tr)
      return false;
    dispatch(tr);
    return true;
...
var HistEvent = class _HistEvent {
  ...
  static fromTransaction(tr, selection4) {
    let effects = none3;
    ...
    if (!effects.length && tr.changes.empty)
      return null;
    return new _HistEvent(tr.changes.invert(tr.startState.doc), effects, void 0, selection4 || tr.startState.selection, none3);
  }
...
function addSelection(branch, selection4) {
  if (!branch.length) {
    return [HistEvent.selection([selection4])];
  } else {
    let lastEvent = branch[branch.length - 1];
    let sels = lastEvent.selectionsAfter.slice(Math.max(0, lastEvent.selectionsAfter.length - MaxSelectionsPerEvent));
    if (sels.length && sels[sels.length - 1].eq(selection4))
      return branch;
    sels.push(selection4);
...
var HistoryState = class _HistoryState {
  ...
  addSelection(selection4, time, userEvent, newGroupDelay) {
    let last2 = this.done.length ? this.done[this.done.length - 1].selectionsAfter : none3;
    if (last2.length > 0 && time - this.prevTime < newGroupDelay && userEvent == this.prevUserEvent && userEvent && /^select($|\.)/.test(userEvent) && eqSelectionShape(last2[last2.length - 1], selection4))
      return this;
    return new _HistoryState(addSelection(this.done, selection4), this.undone, time, userEvent);
  }
  addMapping(mapping) {
  ...
  pop(side, state, onlySelection) {
    ...
    if (branch.length == 0)
      return null;
    let event = branch[branch.length - 1], selection4 = event.selectionsAfter[0] || state.selection;
    if (onlySelection && event.selectionsAfter.length) {
      return state.update({
        selection: event.selectionsAfter[event.selectionsAfter.length - 1],
        annotations: fromHistory.of({ side, rest: popSelection(branch), selection: selection4 }),
      ...
      return state.update({
        ...
        annotations: fromHistory.of({ side, rest: rest2, selection: selection4 }),
        filter: false,
...
function setSel(state, selection4) {
  return state.update({ selection: selection4, scrollIntoView: true, userEvent: "select" });
}
function moveSel({ state, dispatch }, how) {
  let selection4 = updateSel(state.selection, how);
  if (selection4.eq(state.selection, true))
    return false;
  dispatch(setSel(state, selection4));
  return true;
}
...
function cursorByPage(view, forward) {
  ...
  let { state } = view, selection4 = updateSel(state.selection, (range2) => {
    return range2.empty ? view.moveVertically(range2, forward, page.height) : rangeEnd(range2, forward);
  });
  if (selection4.eq(state.selection))
    return false;
  let effect;
  if (page.selfScroll) {
    ...
    if (startPos && startPos.top > scrollTop && startPos.bottom < scrollBottom)
      effect = EditorView.scrollIntoView(selection4.main.head, { y: "start", yMargin: startPos.top - scrollTop });
  }
  view.dispatch(setSel(state, selection4), { effects: effect });
  return true;
}
...
function toMatchingBracket(state, dispatch, extend2) {
  let found = false, selection4 = updateSel(state.selection, (range2) => {
...
function extendSel(view, how) {
  let selection4 = updateSel(view.state.selection, (range2) => {
    let head = how(range2);
    return EditorSelection.range(range2.anchor, head.head, head.goalColumn, head.bidiLevel || void 0);
  });
  if (selection4.eq(view.state.selection))
    return false;
  view.dispatch(setSel(view.state, selection4));
  return true;
}
...
var selectParentSyntax = ({ state, dispatch }) => {
  let selection4 = updateSel(state.selection, (range2) => {
    var _a2;
    let stack = syntaxTree(state).resolveStack(range2.from, 1);
    for (let cur2 = stack; cur2; cur2 = cur2.next) {
      let { node } = cur2;
      if ((node.from < range2.from && node.to >= range2.to || node.to > range2.to && node.from <= range2.from) && ((_a2 = node.parent) === null || _a2 === void 0 ? void 0 : _a2.parent))
...
var simplifySelection = ({ state, dispatch }) => {
  let cur2 = state.selection, selection4 = null;
  if (cur2.ranges.length > 1)
    selection4 = EditorSelection.create([cur2.main]);
  else if (!cur2.main.empty)
    selection4 = EditorSelection.create([EditorSelection.cursor(cur2.main.head)]);
  if (!selection4)
    return false;
  dispatch(setSel(state, selection4));
  return true;
...
var deleteByChar = (target, forward) => deleteBy(target, (range2) => {
  let pos = range2.from, { state } = target, line = state.doc.lineAt(pos), before2, targetPos;
...
var deleteLine = (view) => {
  ...
  let selection4 = updateSel(state.selection, (range2) => view.moveVertically(range2, true)).map(changes);
  view.dispatch({ changes, selection: selection4, scrollIntoView: true, userEvent: "delete.line" });
  return true;
};
...
var Stack = class _Stack {
  ...
  */
  toString() {
    return `[${this.stack.filter((_3, i) => i % 3 == 0).concat(this.state)}]@${this.pos}${this.score ? "!" + this.score : ""}`;
  }
...
function cutAt(tree, pos, side) {
  let cursor4 = tree.cursor(IterMode.IncludeAnonymous);
  cursor4.moveTo(pos);
  for (; ; ) {
    if (!(side < 0 ? cursor4.childBefore(pos) : cursor4.childAfter(pos)))
      for (; ; ) {
        if ((side < 0 ? cursor4.to < pos : cursor4.from > pos) && !cursor4.type.isError)
        ...
        if (side < 0 ? cursor4.prevSibling() : cursor4.nextSibling())
          break;
        if (!cursor4.parent())
...
var LintPanel = class _LintPanel {
  ...
  moveSelection(selectedIndex) {
    ...
    let selection4 = findDiagnostic(field.diagnostics, this.items[selectedIndex].diagnostic);
    if (!selection4)
      return;
    this.view.dispatch({
      selection: { anchor: selection4.from, head: selection4.to },
      scrollIntoView: true,
      effects: movePanelSelection.of(selection4)
    });
  }
  static open(view) {
...
function createLineDialog(view) {
  ...
  function go() {
    ...
    let selection4 = EditorSelection.cursor(docLine.from + Math.max(0, Math.min(col, docLine.length)));
    view.dispatch({
      effects: [dialogEffect.of(false), EditorView.scrollIntoView(selection4.from, { y: "center" })],
      selection: selection4
    });
    view.focus();
  }
...
var matchHighlighter = /* @__PURE__ */ ViewPlugin.fromClass(class {
  ...
  getDeco(view) {
    ...
    for (let part of view.visibleRanges) {
      let cursor4 = new SearchCursor(state.doc, query, part.from, part.to);
      while (!cursor4.next().done) {
        let { from, to } = cursor4.value;
...
var selectWord = ({ state, dispatch }) => {
  let { selection: selection4 } = state;
  let newSel = EditorSelection.create(selection4.ranges.map((range2) => state.wordAt(range2.head) || EditorSelection.cursor(range2.head)), selection4.mainIndex);
  if (newSel.eq(selection4))
    return false;
...
function findNextOccurrence(state, query) {
  ...
  for (let cycled = false, cursor4 = new SearchCursor(state.doc, query, ranges[ranges.length - 1].to); ; ) {
    cursor4.next();
    if (cursor4.done) {
      if (cycled)
        return null;
      cursor4 = new SearchCursor(state.doc, query, 0, Math.max(0, ranges[ranges.length - 1].from - 1));
      cycled = true;
    } else {
      if (cycled && ranges.some((r) => r.from == cursor4.value.from))
        continue;
      if (fullWord) {
        let word2 = state.wordAt(cursor4.value.from);
        if (!word2 || word2.from != cursor4.value.from || word2.to != cursor4.value.to)
          continue;
      }
      return cursor4.value;
    }
  }
}
...
var StringQuery = class extends QueryType2 {
  ...
  nextMatch(state, curFrom, curTo) {
    let cursor4 = stringCursor(this.spec, state, curTo, state.doc.length).nextOverlapping();
    if (cursor4.done)
      cursor4 = stringCursor(this.spec, state, 0, curFrom).nextOverlapping();
    return cursor4.done ? null : cursor4.value;
  }
  ...
  prevMatchInRange(state, from, to) {
    for (let pos = to; ; ) {
      ...
      let cursor4 = stringCursor(this.spec, state, start, pos), range2 = null;
      while (!cursor4.nextOverlapping().done)
        range2 = cursor4.value;
      if (range2)
        return range2;
      if (start == from)
        return null;
  ...
  matchAll(state, limit) {
    let cursor4 = stringCursor(this.spec, state, 0, state.doc.length), ranges = [];
    while (!cursor4.next().done) {
      if (ranges.length >= limit)
        return null;
      ranges.push(cursor4.value);
    }
    return ranges;
  }
  highlight(state, from, to, add2) {
    let cursor4 = stringCursor(this.spec, state, Math.max(0, from - this.spec.unquoted.length), Math.min(to + this.spec.unquoted.length, state.doc.length));
    while (!cursor4.next().done)
      add2(cursor4.value.from, cursor4.value.to);
  }
};
...
var RegExpQuery = class extends QueryType2 {
  nextMatch(state, curFrom, curTo) {
    let cursor4 = regexpCursor(this.spec, state, curTo, state.doc.length).next();
    if (cursor4.done)
      cursor4 = regexpCursor(this.spec, state, 0, curFrom).next();
    return cursor4.done ? null : cursor4.value;
  }
  prevMatchInRange(state, from, to) {
    for (let size2 = 1; ; size2++) {
      ...
      let cursor4 = regexpCursor(this.spec, state, start, to), range2 = null;
      while (!cursor4.next().done)
        range2 = cursor4.value;
  ...
  getReplacement(result2) {
    return this.spec.unquote(this.spec.replace).replace(/\$([$&\d+])/g, (m, i) => i == "$" ? "$" : i == "&" ? result2.match[0] : i != "0" && +i < result2.match.length ? result2.match[i] : m);
  }
  matchAll(state, limit) {
    let cursor4 = regexpCursor(this.spec, state, 0, state.doc.length), ranges = [];
    while (!cursor4.next().done) {
      if (ranges.length >= limit)
        return null;
      ranges.push(cursor4.value);
  ...
  highlight(state, from, to, add2) {
    let cursor4 = regexpCursor(this.spec, state, Math.max(
      0,
      from - 250
...
var findNext = /* @__PURE__ */ searchCommand((view, { query }) => {
  ...
  if (!next)
    return false;
  let selection4 = EditorSelection.single(next.from, next.to);
  let config2 = view.state.facet(searchConfigFacet);
  view.dispatch({
    selection: selection4,
    effects: [announceMatch(view, next), config2.scrollToMatch(selection4.main, view)],
...
var findPrevious = /* @__PURE__ */ searchCommand((view, { query }) => {
  ...
  if (!prev)
    return false;
  let selection4 = EditorSelection.single(prev.from, prev.to);
  let config2 = view.state.facet(searchConfigFacet);
  view.dispatch({
    selection: selection4,
    effects: [announceMatch(view, prev), config2.scrollToMatch(selection4.main, view)],
    userEvent: "select.search"
  });
  selectSearchInput(view);
  return true;
...
var replaceNext = /* @__PURE__ */ searchCommand((view, { query }) => {
  ...
  if (!next)
    return false;
  let changes = [], selection4, replacement;
  let effects = [];
  ...
  if (next) {
    ...
    selection4 = EditorSelection.single(next.from - off, next.to - off);
    effects.push(announceMatch(view, next));
    effects.push(state.facet(searchConfigFacet).scrollToMatch(selection4.main, view));
  }
  view.dispatch({
    changes,
    selection: selection4,
...
function emulatedSet(keys3) {
  var hash2 = {};
  for (var l = keys3.length, i = 0; i < l; ++i)
    hash2[keys3[i]] = true;
  return {
...
function collectNonEnumProps(obj, keys3) {
  ...
  if (has(obj, prop) && !keys3.contains(prop))
    keys3.push(prop);
  while (nonEnumIdx--) {
...
function template(text, settings, oldSettings) {
  ...
  var template2 = function(data) {
    return render2.call(this, data, _);
  };
...
function flatten2(input, depth, strict, output) {
  output = output || [];
  if (!depth && depth !== 0) {
    depth = Infinity;
  } else if (depth <= 0) {
  ...
  for (var i = 0, length = getLength_default(input); i < length; i++) {
    ...
    if (isArrayLike_default(value) && (isArray_default(value) || isArguments_default(value))) {
      if (depth > 1) {
        flatten2(value, depth - 1, strict, output);
        idx = output.length;
      } else {
...
var bindAll_default = restArguments(function(obj, keys3) {
  keys3 = flatten2(keys3, false, false);
...
function createReduce(dir) {
  var reducer = function(obj, iteratee2, memo, initial2) {
    var _keys = !isArrayLike_default(obj) && keys2(obj), length = (_keys || obj).length, index = dir > 0 ? 0 : length - 1;
    if (!initial2) {
...
var omit_default = restArguments(function(obj, keys3) {
  ...
  if (isFunction_default(iteratee2)) {
    ...
    keys3 = map(flatten2(keys3, false, false), String);
...
function flatten3(array, depth) {
  return flatten2(array, depth, false);
}
...
// node_modules/cm6-theme-nord/dist/index.js
var base00 = "#2e3440";
var base01 = "#3b4252";
var base02 = "#434c5e";
var base03 = "#4c566a";
...
var base0F = "#b48ead";
var invalid = "#d30102";
var darkBackground = "#252a33";
var highlightBackground = base03;
var background = base00;
var tooltipBackground = base01;
var selection = base03;
var cursor = base04;
var nordTheme = /* @__PURE__ */ EditorView.theme({
  "&": {
    color: base04,
...
var nordHighlightStyle = /* @__PURE__ */ HighlightStyle.define([
...
// node_modules/cm6-theme-solarized-light/dist/index.js
var base002 = "#657b83";
var base012 = "#586e75";
var base022 = "#073642";
var base032 = "#002b36";
...
var solarizedLightHighlightStyle = /* @__PURE__ */ HighlightStyle.define([
...
var base0F2 = "#b48ead";
var invalid3 = "#d30102";
var darkBackground3 = base062;
var highlightBackground3 = darkBackground3;
var background3 = "#ffffff";
var tooltipBackground3 = base013;
var selection3 = darkBackground3;
var cursor3 = base013;
var basicLightTheme = /* @__PURE__ */ EditorView.theme({
  ...
  ".cm-cursor, .cm-dropCursor": { borderLeftColor: cursor3 },
  "&.cm-focused .cm-selectionBackground, .cm-selectionBackground, .cm-content ::selection": { backgroundColor: selection3 },
  ".cm-panels": { backgroundColor: darkBackground3, color: base033 },
  ".cm-panels.cm-panels-top": { borderBottom: "2px solid black" },
  ".cm-panels.cm-panels-bottom": { borderTop: "2px solid black" },
  ".cm-searchMatch": {
    backgroundColor: "#72a1ff59",
    outline: `1px solid ${base033}`
  },
  ".cm-searchMatch.cm-searchMatch-selected": {
    backgroundColor: base052
  },
  ".cm-activeLine": { backgroundColor: highlightBackground3 },
  ".cm-selectionMatch": { backgroundColor: base052 },
  "&.cm-focused .cm-matchingBracket, &.cm-focused .cm-nonmatchingBracket": {
    outline: `1px solid ${base033}`
  },
  "&.cm-focused .cm-matchingBracket": {
    backgroundColor: base062
  },
  ".cm-gutters": {
    backgroundColor: base062,
    color: base003,
  ...
  ".cm-activeLineGutter": {
    backgroundColor: highlightBackground3
  },
  ".cm-foldPlaceholder": {
    backgroundColor: "transparent",
    border: "none",
  ...
  ".cm-tooltip": {
    border: "none",
    backgroundColor: tooltipBackground3
  },
  ".cm-tooltip .cm-tooltip-arrow:before": {
    borderTopColor: "transparent",
    borderBottomColor: "transparent"
  ...
  ".cm-tooltip .cm-tooltip-arrow:after": {
    borderTopColor: tooltipBackground3,
    borderBottomColor: tooltipBackground3
  },
  ".cm-tooltip-autocomplete": {
    "& > ul > li[aria-selected]": {
      backgroundColor: highlightBackground3,
...
var basicLightHighlightStyle = /* @__PURE__ */ HighlightStyle.define([
  { tag: tags.keyword, color: base0A2 },
  {
    tag: [tags.name, tags.deleted, tags.character, tags.propertyName, tags.macroName],
    color: base0C2
  },
  { tag: [tags.variableName], color: base0C2 },
  { tag: [/* @__PURE__ */ tags.function(tags.variableName)], color: base0A2 },
  { tag: [tags.labelName], color: base092 },
  {
    tag: [tags.color, /* @__PURE__ */ tags.constant(tags.name), /* @__PURE__ */ tags.standard(tags.name)],
    color: base0A2
  },
  { tag: [/* @__PURE__ */ tags.definition(tags.name), tags.separator], color: base0E2 },
  { tag: [tags.brace], color: base072 },
  {
    tag: [tags.annotation],
    color: invalid3
  },
  ...
  { tag: [tags.meta], color: base082 },
  { tag: [tags.comment], color: base023, fontStyle: "italic" },
  { tag: tags.strong, fontWeight: "bold", color: base0A2 },
  { tag: tags.emphasis, fontStyle: "italic", color: base0A2 },
  { tag: tags.strikethrough, textDecoration: "line-through" },
  { tag: tags.heading, fontWeight: "bold", color: base0A2 },
  { tag: /* @__PURE__ */ tags.special(tags.heading1), fontWeight: "bold", color: base0A2 },
  { tag: tags.heading1, fontWeight: "bold", color: base0A2 },
  {
    tag: [tags.heading2, tags.heading3, tags.heading4],
    fontWeight: "bold",
    color: base0A2
  ...
  { tag: [tags.atom, tags.bool, /* @__PURE__ */ tags.special(tags.variableName)], color: base0C2 },
  {
    tag: [tags.processingInstruction, tags.inserted],
    color: base072
  },
  ...
  { tag: tags.invalid, color: base023, borderBottom: `1px dotted ${invalid3}` }
]);
var basicLight = [
  basicLightTheme,
...
// src/widget_code_input/index.js
function render(context) {
  let el = context.el;
  let model = context.model;
  model.attributes = { function_body_id: "fid" };
  const cssStyles = "<style>            .forced-indent {width: 40em;}            .CodeMirror {border: 1px solid #aaa; height: auto;}            .CodeMirror.widget-code-input-signature {border-bottom: 0px;}            .CodeMirror.widget-code-input-docstring {border-top: 0px;}            .CodeMirror.widget-code-input-body {border-top: 0px;}        </style>";
  const theTextareaId = model.attributes["function_body_id"];
  let textArea = document.createElement("textArea");
  el.appendChild(textArea);
  textArea.outerHTML = cssStyles + '<textarea id="' + theTextareaId + '-signature" class="textwidgetclassname"></textarea><textarea id="' + theTextareaId + '-docstring"></textarea><textarea id="' + theTextareaId + '-body"></textarea>';
  console.log(el);
  ...
  defer_default(() => {
    const HTMLTextAreaElement = document.querySelector("#fid-signature");
    const htextArea = HTMLTextAreaElement;
    var gutter2 = new Compartment();
    ...
    const mySignatureCodeMirror = editorFromTextArea(
    ...
    var myBodyCodeMirror = editorFromTextArea(document.getElementById(theTextareaId + "-body"), [
      gutter2.of(lineNumbers()),
      bodyUpdateListenerCompartment.of([]),
      highlightActiveLineGutter(),
      highlightActiveLineGutter(),
      highlightSpecialChars(),
      history(),
      ...
      python(),
      theme_compartment_body.of(basicLight)
    ]);
    ...
    function signatureValueChanged() {
      const newSignature = "def " + model.get("function_name") + "(" + model.get("function_parameters") + "):";
    ...
    function themeChanged() {
      ...
      if (theme_string in themeMap) {
        theme3 = themeMap[theme_string];
      } else {
        throw new Error("Specified code theme is not supported.");
      }
      console.log("theme changed, theme= %s", theme_string);
      mySignatureCodeMirror.dispatch({
        effects: theme_compartment_sig.reconfigure(theme3)
      });
      myDocstringCodeMirror.dispatch({
        effects: theme_compartment_doc.reconfigure(theme3)
      });
      myBodyCodeMirror.dispatch({
        effects: theme_compartment_body.reconfigure(theme3)
      });
    }
    ...
    var theme2 = model.get("code_theme");
...
var widget_code_input_default = { render };
export {
  widget_code_input_default as default
};
